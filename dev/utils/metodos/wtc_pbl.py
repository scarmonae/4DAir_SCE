   ...
   - numpy.ndarray: Los coeficientes de covarianza wavelet en cada escala y altura (arreglo 2D).
    ...
    for scale in scales:
        ...
        for shift in range_array:
            ...
            cov = (1/scale) * np.sum(signal_profile * wavelet) * delta_r
...
# def wavelet_covariance_transform(signal_profile, range_array, scales):
    ...
    dz = (range_array.max() - range_array.min()) / (n - 1)
    ...
    for scale in tqdm(scales, desc="Iterating over dilatations..."):
        ...
        for shift in range_array:
           ...
           # Calcular la covarianza (producto punto)
            integrando = signal_profile * wavelet
            # Aproximar la integral utilizando la regla del trapecio
            integral_result = 0
            n = len(integrando)

            integral_result = np.sum((integrando[:-1] + integrando[1:]) / 2) * dz

            covariance.append(integral_result) 
        coefficients.append(covariance)
...
def process_lidar_profile(signal_profile, range_array, scales, global_mean, global_std, plot_wct=False):
    """
    ...
    signal_profile = (signal_profile - global_mean) / global_std
    ...
    if plot_wct:
        ...
        plt.figure(figsize=(6, 8))
        plt.plot(signal_profile, range_array, label='LIDAR Signal')
        plt.plot(wavelet_at_max, range_array, label='Haar Wavelet at Max Covariance')
...
def process_lidar_dataset(dataset, signal_variable, range_variable, time_variable, scales):
    """
    Procesa un Dataset de xarray que contiene perfiles LIDAR en el tiempo para estimar la altura de la PBL en cada instante.
    """
    # Extraer todos los perfiles de señal como un arreglo 2D (tiempo x altura)
    all_signal_profiles = dataset[signal_variable].values  # Debería ser un arreglo 2D: tiempo x altura

    # Calcular la mediana y MAD estándar globales (como estimadores robustos de la tendencia central y la dispersión de los datos)
    global_median = np.median(all_signal_profiles)

    # Calcular el MAD
    mad = np.median(np.abs(all_signal_profiles - global_median))

    # Escalar el MAD para hacerlo consistente con la desviación estándar
    scaled_mad = mad * 1.4826  # Factor de escala para distribuciones normales

    # Calcular los residuos (distancias desde la mediana en unidades de MAD)
    residuals = (all_signal_profiles - global_median) / mad

    # Calcular el percentil 97.5 de los residuos
    k = np.percentile(residuals, 97.5)

    threshold = global_median + k * mad
    ...
    for i, t in tqdm(enumerate(times), desc="Procesando perfiles..."):
        signal_profile = dataset[signal_variable].sel({time_variable: t}).values

        # Pasar la media y desviación estándar globales a la función
        plot_wct = (i % 100 == 0)
        pbl_results = process_lidar_profile(signal_profile, range_array, scales, global_median, scaled_mad, plot_wct=plot_wct)

        pbl_heights_sum.append(pbl_results['pbl_height_sum'])
        pbl_heights_max.append(pbl_results['pbl_height_max'])

    # Crear un Dataset con ambas estimaciones de altura de la PBL
